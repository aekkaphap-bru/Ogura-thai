...
namespace OCTWEB_NET45.Controllers.LeaveFormer
{
    [Authorize]
    public class LeaveFormerController : Controller
    {
        ...
        //
        // GET: /Leave/LeaveList
        [CustomAuthorize(60)]//Can see Work Leave All
        public ActionResult LeaveList(SearchLeave model)
        {
            try
            {
                ...
                //Select Employee or Former
                List<EmployeeModel> query_emp = new List<EmployeeModel>();
                query_emp = db.Former_EmpList.Select(s => new EmployeeModel
                {
                    EmpID = s.EmpID,
                    Title_EN = s.Title_EN,
                    NName_EN = s.NName_EN,
        ...
        //        leave_event.title = l.i + 1 + "-" + l.value.fname + " " + l.value.lname + " (" + l.value.nickname + ")" + "-" + l.value.typeleave;
        ...
        //        if (l.value.typeleave == "Sick Cert ( ลาป่วย มีใบรับรองแพทย์ )" || l.value.typeleave == "Sick ( ลาป่วย ไม่มีใบรับรองแพทย์ )")
        //        {
        //            //leave_event.backgroundColor = "#E77471";
        //            //leave_event.borderColor = "#F75D59";
        //            //leave_event.textColor = "#FFFFFF";
        //        }
        //        else
        //        {
        //            //leave_event.backgroundColor = "#AAF0D1";//"#36F57F";
        //            //leave_event.borderColor = "#66CDAA";
        ...
        public ActionResult Create(FormCollection form, LeaveModel model)
        {
            try
            {
                ...
                {
                    var former_model = db.Former_EmpList.Where(x => x.EmpID == model.emp_id).FirstOrDefault();
                    if (former_model == null)
                    {
                    ...
                    if (former_model.EmpStatus == "Employee Ogura Clutch")
                    {
        ...
        public ActionResult Edit(int id)
        {
            try
            {
                ...
                emp = db.Former_EmpList.Where(x => x.EmpID == model.emp_id)
        ...
        public JsonResult GetEmpDetail(int emp_id)
        {
            ...
            {
                EmployeeModel emp = new EmployeeModel();
                emp = db.Former_EmpList.Where(x => x.EmpID == emp_id)
                       .Select(s => new EmployeeModel
                       {
                           EmpID = s.EmpID,
                           FName_EN = s.FName_EN,
        ...
        public void ExportToCsv(List<LeaveModel> model)
        {
            try
            {
                ...
                response.AddHeader("content-disposition", "attachment;filename=Former_Employee_Leave.CSV ");
        ...
        public void ExportToCsvSum(List<LeaveModel> model,string search_startdate,string search_enddate)
        {
            try
            {
                ...
                emp = db.Former_EmpList.Where(x => emp_id_list.Contains(x.EmpID))
                   .Select(s => new EmployeeModel
                   {
                       EmpID = s.EmpID,
                       Title_EN = s.Title_EN,
                ...
                */
                ...
                Response.Charset = "windows-874";
                Response.ContentEncoding = System.Text.Encoding.GetEncoding(874);
                response.AddHeader("content-disposition", "attachment;filename=Former_Employee_Leave_Summary.CSV ");
                response.ContentType = "text/plain";
                response.Write(sb.ToString());
                response.End();
