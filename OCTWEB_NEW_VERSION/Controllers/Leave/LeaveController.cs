...
namespace OCTWEB_NET45.Controllers.Leave
{
    ...
    public class LeaveController : Controller
    {
        ...
        //
        // GET: /Leave/LeaveList
        [CustomAuthorize(63)]//Leave Setup
        public ActionResult LeaveList(SearchLeave model)
        {
            try
            {
               ...
               /* foreach (var i in emp_leave_list)
                ...
                if (Session["USE_Id"] != null)
                {
                    ...
                    //Add Holiday leave report(62)
                    int rights_62 = db.UserRights.Where(x => x.USE_Id == use_id && x.RIH_Id == 62).Count();
                   
                    model.rights_62 = rights_62 > 0 ? true : false;
                }
            ...
            {
                ViewBag.errorMessage = String.Format("Error: Get /Leave/LeaveList {0}", ex.ToString());
                return View("Error");
            }  
        }
        ...
        public SearchLeave FullCalendarSet(SearchLeave model)
        {
            ...
            foreach (var h in holiday_list)
            {
                if (h.start_date.HasValue && h.end_date.HasValue)
                {
                    ...
                    leave_event.id = h.id;
                    leave_event.title = h.text;
                    leave_event.start = h.start_date.Value.ToString("yyyy-MM-dd");
                    leave_event.end = h.end_date.Value.AddDays(1).ToString("yyyy-MM-dd");
                    leave_event.type = "holiday";
                    leave_event.allDay = true;
                    leave_event.color = "";
                    leave_event.textColor = "";
                    leave_event.backgroundColor = "";
                    leave_event.borderColor = "";
            ...
            foreach (var l in emp_leave_list.Select((value, i) => new { i, value }))
            {
                ...
                foreach (var workDay in workDays)
                {
                    ...
                    {
                        var leave_event = new EventViewModel();
                        leave_event.id = LeaveEvents.Count + 1;
                        leave_event.title = l.value.emp_id + " " + l.value.fname + " " + l.value.lname + " (" + l.value.nickname + ")";
                        leave_event.start = workDay.ToString("yyyy-MM-ddTHH:mm:ss");
                        leave_event.end = workDay.ToString("yyyy-MM-ddTHH:mm:ss");
                        leave_event.type = "leave";
                        leave_event.allDay = false;
                        leave_event.display = "none";
                        leave_event.typeleave = l.value.typeleave;
                        LeaveEvents.Add(leave_event);
                    }
                }
            }
        ...
        public ActionResult LeaveList(FormCollection form,SearchLeave model)
        {
            try
            {
                ...
                {
                    string search_startdate = start_date.HasValue? start_date.Value.ToString("dd-MM-yyyy"):"";
                    string search_enddate = end_date.HasValue? end_date.Value.ToString("dd-MM-yyyy"):"";
            ...
            {
                ViewBag.errorMessage = String.Format("Error: Post /Leave/LeaveList {0}", ex.ToString());
                return View("Error");
            }
        }
        ...
        [HttpPost]
        [CustomAuthorize(63)]//Leave Setup
        public ActionResult Create(FormCollection form, LeaveModel model)
        {
            try
            {
                //Select Type of Leave
        ...
        [HttpPost]
        [CustomAuthorize(63)]//Leave Setup
        public ActionResult Edit(FormCollection form, LeaveModel model)
        {
            try
            {
                //Select Type of Leave
        ...
        public JsonResult GetEmpDetail(int emp_id)
        {
            ...
            {
                EmployeeModel emp = new EmployeeModel();
                emp = db.EmpLists.Where(x => x.EmpID == emp_id)
                    .Select(s => new EmployeeModel 
                    {
                        EmpID = s.EmpID,
                        FName_EN = s.FName_EN,
        ...
        //
        // GET: /Leave/Setting
        [CustomAuthorize(62)]//Add Holiday leave report
        public ActionResult Setting()
        {
            try
            {
                ...
                //Select Holiday
                List<Emp_Holiday_sickleave> query_ho = db.Emp_Holiday_sickleave.OrderByDescending(o => o.date_start).ToList();
                List<SelectListItem> selectHoliday = query_ho
                    .OrderByDescending(o=>o.date_start)
                    .Select(s => new SelectListItem { 
                        Value = s.Id.ToString(),
                        Text = String.Concat(s.date_start.Value.ToString("yyyy-MM-dd"), " : ", s.Detail)
                    }
                    ).ToList();


                model.SelectHoliday = selectHoliday;
          
                return View(model);
            }
            catch (Exception ex)
            {
                ViewBag.errorMessage = String.Format("Error: GET /Leave/Setting {0} ", ex.ToString());
                return View("Error");
            }
        }
        //
        ...
        [HttpPost]
        [CustomAuthorize(62)]//Add Holiday leave report
        public ActionResult Setting(LeaveSettingModel model)
        {
            try
            {
                //Select Holiday
                ...
                if(check == "add")
                {
                    if (ModelState.IsValid)
                    {
                        Emp_Holiday_sickleave ho = new Emp_Holiday_sickleave();
                        ho.date_start = model.holiday_date;
                        ho.date_end = model.holiday_date_end;
                        ho.Detail = model.detail;
                        db.Emp_Holiday_sickleave.Add(ho);
                        db.SaveChanges();
                        TempData["shortMessage"] = String.Format("Created successfully, {0} . ", model.detail);
                ...
                if (check == "delete")
                {
                    List<int> holiday_list = model.holiday_id;
                    if(holiday_list.Any())
                    {
                        List<Emp_Holiday_sickleave> ho_list = db.Emp_Holiday_sickleave
                            .Where(x => holiday_list.Contains(x.Id)).ToList();
                        if(ho_list.Any())
                        {
                            db.Emp_Holiday_sickleave.RemoveRange(ho_list);
                            db.SaveChanges();
                            TempData["shortMessage"] = String.Format("Deleted successfully ");
                            return RedirectToAction("Setting");
                        }
                    }
                }
            ...
            {
                 ViewBag.errorMessage = String.Format("Error: Post /Leave/Setting ", ex.ToString());
                return View("Error");
            }
        }
        ...
        public void ExportToCsvSum(List<LeaveModel> model,string search_startdate,string search_enddate)
        {
            try
            {
                ...
                emp = db.EmpLists.Where(x => emp_id_list.Contains(x.EmpID))
                    .Select(s => new EmployeeModel
                    {
                        EmpID = s.EmpID,
                        Title_EN = s.Title_EN,
                ...
                List<ExportToCsvSumModel> forexport = emp
